...
const MakeOffer = ({ isListed, selectedNft, listings, marketPlaceModule }) => {
	...
	const confirmPurchase = (toastHandler = toast) => toastHandler.success("Purchase successful!", {
		style: {
			background: "#04111d",
			color: "#fff"
		}
	...
	return (
		<div className="bg-[#303339] h-20 w-full flex items-center px-12 rounded-lg border-[#151c22] border">
			...
			{isListed === "true" ? (
				<>
					<div onClick={() => { enableButton ? buyItem(selectedMarketNft.id, 1) : null }} className={`${style.button} bg-[#2081e2] hover:bg-[#42a0ff]`}>
